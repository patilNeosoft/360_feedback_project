...
{
    public class GetUserRoleByIdTests
    {
        ...
        [Fact]
        public async Task Handle_ValidGetUserRoleById()
        {
            var handler = new GetUserRoleByIdQueryHandler(_mapper, _mockUserRoleRepository.Object);
            var result = await handler.Handle(new GetUserRoleByIdQuery(), CancellationToken.None);
            result.ShouldBeOfType<GetUserRoleByIdVm>();
